...
No *balancear(No *node) {
  ...
  if (fb > 1) {
    No *novo_node = node->esq;
    if (fator_balanceamento(node->esq) == -1) {
      rotacao_simples_esquerda(node->esq);
    }
    rotacao_simples_direita(node);
    return novo_node;
  } else {
    if (fb < -1) {
      No *novo_node = node->dir;
      if (fator_balanceamento(node->dir) == -1) {
        rotacao_simples_direita(node->dir);
      }
      rotacao_simples_esquerda(node);
      return novo_node;
    } else {
      return node;
    }
